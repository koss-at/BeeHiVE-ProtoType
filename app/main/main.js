...
async function createWindow() {
  ...
  if (isDev) {
    ...
    const indexFile = path.join(__dirname, "../renderer/dist/index.html");
    await win.loadFile(indexFile);
  }
...
ipcMain.handle("rename:execute", async (_evt, items) => {
  ...
  const lines = [];
  lines.push(
    `[${new Date().toISOString()}] EXECUTE BEGIN count=${items.length}`,
  );
  try {
    ...
    for (const it of applied.reverse()) {
      try {
        await fsp.rename(it.to, it.from);
      } catch {}
    }
...
ipcMain.handle("debug:exportDiagnostics", async () => {
  ...
  const file = path.join(
    dir,
    `BeeHiVE_snapshot_${new Date().toISOString().replace(/[:]/g, "-")}.json`,
  );
  const payload = {
    appInfo: {
